...
PRODUCT_NAME_GENERATION_SYSTEM_PROMPT = """당신은 네이버 스마트스토어 상품명 SEO 최적화 전문가입니다. 사용자가 제공하는 추출된 키워드 리스트와 그중 핵심이 되는키워드 그리고 필수 입력 키워드(브랜드명,재료,수량), 상위 상품명의 길이 통계 정보를 바탕으로, 아래 가이드라인에 따라 네이버 쇼핑 검색 알고리즘에 최적화된 상품명을 생성하고, 해당 상품명이 어떻게 최적화되었는지 그 방안을 상세히 설명해주세요."""

DEFAULT_PRODUCT_NAME_GENERATION_PROMPT = """[사용자 입력 정보]
1. 선택된 추출 키워드 리스트: {selected_keywords}
2. 선택된 추출 키워드 리스트에서 중심이 되는 핵심 키워드: {core_keyword}
3. 필수 입력 키워드: 
   - 브랜드명: {brand}
   - 재료(형태): {material}
   - 수량(무게): {quantity}
...
1. 선택된 핵심 키워드 리스트: {selected_keywords}
2. 상위 상품명 길이 통계:  
• 상품명 길이 최적화: 네이버는 상품명이 너무 길거나 너무 짧은 것 모두 좋지 않다고 명시하며, 간결하되 사용자에게 필요한 충분한 정보를 담으라고 권장합니다. 제공해주는 상위 판매자의 평균 상품명 수를 참고하여 맞추는 것을 권장합니다.
• 띄어쓰기 및 붙여쓰기: 네이버 검색 시스템은 띄어쓰기 분석이 자동 반영됩니다. 복합 키워드의 경우 띄어쓰기를 하지 않는 것이 알고리즘의 혼란을 줄여 노출에 유리할 수 있습니다 (예: '페이스롤러'는 붙여 쓰는 것이 유리). 하지만 사용자(소비자)의 편의를 위한 자연스러운 가독성도 함께 고려합니다. '핸드크림'과 '핸드 띄고 크림'처럼 띄어쓰기에 따라 다른 키워드로 인식될 수 있는 경우를 인지합니다.
• 브랜드 및 제조사: 제공된 키워드 중에 브랜드명이 있다면 상품명 제일 앞에 브랜드명을 입력합니다.
• 재료(형태) 및 수량(무게): 제공된 키워드에서 재료 및 수량이 있다면 상품명 제일 마지막에 입력합니다. 예) 슈퍼츄 오븐베이크사료 비프 1.3kg, 슈퍼츄 터키츄 링s 20개

[출력 형식]
8-10개의 상품명을 아래 형식으로 생성해주세요:
